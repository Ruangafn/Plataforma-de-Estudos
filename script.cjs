        ...
        function deleteCurrentProfile() {
            const list = getProfilesList();
            if (list.length <= 1) {
                alert("Você possui apenas 1 perfil ativo. Não é possível excluir o único perfil existente. Crie outro perfil primeiro se desejar remover este.");
                return;
            }
        ...
        function deleteCurrentCycle() {
            if (!appData.cycle || !appData.cycle.active) {
                alert("Não há nenhum ciclo ativo neste perfil para apagar.");
                return;
            }
        ...
        function importBackupJSON(event) {
            ...
            reader.onload = function(e) {
                try {
                    ...
                    saveData();
                    renderAll();
                    alert("Backup restaurado com sucesso para o perfil atual!");
                } catch(err) {
                ...
                <div class="${rowClass}" data-id="${t.id}" draggable="true" ondragstart="handleCustTopicDragStart(event)" ondragover="handleCustTopicDragOver(event)" ondrop="handleCustTopicDrop(event)" ondragenter="handleCustTopicDragEnter(event)" ondragleave="handleCustTopicDragLeave(event)">
                    ...
                    <div class="cust-status-group">
                        ...
                        <select class="status-select" style="background: var(--bg-main);" onchange="handleCustStatusChange(this)">
                            ...
                            <option value="unlisted" ${t.status==='unlisted'?'selected':''}>⏸ Não Listado</option>
                        </select>
                    </div>
        ...
        function openCadernoLink(topicId) {
            ...
            if (!raw) return alert("Nenhum link cadastrado para este assunto.");
        ...
        function saveQStats(subId, topicId) {
            ...
            if (topic.qCorrect > topic.qTotal) {
                alert("Atenção: Os acertos não podem ultrapassar o total de questões.");
                return;
            }

            saveData(); 
            renderAll(); 
            alert("Estatísticas e link salvos com sucesso!");
        }
        ...
        function saveExtraTask() {
            ...
            if(!subId || !topicId) return alert("Selecione a disciplina e o assunto.");
            if(time > 1.5) return alert("Tarefas extras não devem exceder 1,5h (90 min) de acordo com o padrão do ciclo.");
            if(qCor > qTot) return alert("Acertos não podem ser maiores que o total de questões.");

            const sub = appData.subjects.find(s => s.id === subId);
            const topic = sub ? sub.topics.find(t => t.id === topicId) : null;
            if (!sub || !topic) return alert("Erro ao identificar matéria/assunto.");
            ...
            closeExtraTaskModal();
            renderAll();
            alert("Tarefa Extra adicionada com sucesso ao seu dia ativo!");
        }
        ...
        function submitQuestionsAndFinish() {
            ...
            if (correct > total) return alert("Acertos não podem ser maiores que o total!");
        ...
        function generateMacroCycle() {
            ...
            if(simTopics.length === 0) return alert("Não há assuntos marcados como Pendentes em disciplinas ativas para gerar o ciclo! Verifique na aba 'Disciplinas & Fases'.");
            ...
            renderAll(); 
            switchTab('dashboard');
            alert(`Ciclo gerado com sucesso! Foram planejados ${allDays.length} dias de estudo com respeito aos seus limites diários.`);
        }
        ...
        function saveCycleSettings() {
            ...
            saveData();
            closeCycleSettingsModal();
            alert("Configurações do ciclo salvas com sucesso!");
        }
        ...
        function handleDayDrop(e, targetDayIndex) {
            ...
            if (appData.cycle.completedDays && appData.cycle.completedDays[targetDayIndex]) {
                alert("Este dia já foi concluído e não pode receber novas tarefas.");
                return;
            }

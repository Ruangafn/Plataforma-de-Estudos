...
html = html.replace(
    /window\.scrollTo\(0, 0\);\s+\}\s+\/\/ --- DRAG AND DROP ---/g,
...
html = html.replace(
    /if \(!confirm\(\`Atenção: Tem certeza de que deseja excluir permanentemente o perfil[\s\S]*?return;\s+\}\s+try \{/g,
...
html = html.replace(
    /if \(!confirm\("Tem certeza que deseja apagar o ciclo atual[\s\S]*?return;\s+\}\s+appData\.cycle = \{/g,
...
html = html.replace(
    /renderCustomizeList\(\);\s+\}\s+function handleCustTopicDragStart/g,
    `renderCustomizeList();\n            });\n        }\n\n        function handleCustTopicDragStart`
);
...
html = html.replace(
    /if \(!confirm\("⚠️ ATENÇÃO: Esta ação é irreversível![\s\S]*?return;\s+\}\s+const confirmCode = prompt\([\s\S]*?try \{/g,
...
html = html.replace(
    /alert\("Todos os dados foram excluídos e o sistema foi restaurado para o padrão inicial\."\);\s+\}/g,
